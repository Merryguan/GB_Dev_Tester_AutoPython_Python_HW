"""
 ...
 Условие:
 Создайте программу,
 которая реализует игру «Крестики-нолики».
 Для этого напишите:
 1. Класс, который будет описывать поле игры.
 class Board:
 - Класс поля, который создаёт у себя экземпляры клетки.
 - Пусть класс хранит информацию о состоянии поля
   (это может быть список из девяти элементов).
...
class Board:
    ...
    def display_board(self):
        ...
        for i in range(0, 9, 3):
            print(f"| {self.cells[i].symbol} | "
                  f"{self.cells[i + 1].symbol} | "
                  f"{self.cells[i + 2].symbol} |")
            print("-------------")
    ...
    def check_game_over(self):
        ...
        for pos in win_positions:
            if (self.cells[pos[0]].symbol != " "
                    and self.cells[pos[0]].symbol == self.cells[pos[1]].symbol == self.cells[pos[2]].symbol):
                return True
        return False
        ...
        while True:
            try:
                move = int(input(f"{self.name}, "
                                 f"введите номер клетки для вашего хода (1-9): "))
                if move < 1 or move > 9:
                    raise ValueError
                return move
            except ValueError:
