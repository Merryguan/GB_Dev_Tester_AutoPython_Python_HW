"""
 ...
 - прожить один день
   (выбирает одно действие согласно описанному ниже приоритету и выполняет его).
 ...
 Если сытость человека становится меньше нуля,
 человек умирает.
 ...
 и создайте двух людей, живущих в одном доме.
...
class Human:
    ...
    def eat(self):
        """Метод, который увеличивает сытость человека
        и уменьшает количество еды в доме."""
        if self.house.food >= 10:
            self.hunger += 10
            self.house.food -= 10
            print(f"{self.name} поел. "
                  f"Сытость увеличилась до {self.hunger}, "
                  f"еда уменьшилась до {self.house.food}.")
        else:
            print(f"{self.name} хотел поесть, но в доме недостаточно еды!")

    def work(self):
        """Метод, который уменьшает сытость человека
        и увеличивает количество денег в доме."""
        self.hunger -= 10
...
try:
    for day in range(1, 366): # Моделируем 365 дней print(f"\nДень {day}")
