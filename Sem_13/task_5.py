"""
 ...
 Задание №5. Валидатор Пользовательских Данных.

 Условие:
 Создайте класс User,
 который содержит атрибуты name, email, и age.
 Необходимо убедиться, что:
 - Имя состоит из хотя бы двух слов,
   каждое из которых начинается с заглавной буквы.
 - Электронная почта содержит символ @
   и точку . после @.
 - Возраст — это положительное целое число,
   не меньше 0 и не больше 120.
...
class NameError(Exception):
    def __init__(self):
        super().__init__("Имя должно состоять из хотя бы двух слов, "
                         "каждое из которых начинается с заглавной буквы.")
...
class User:
    ...
    @name.setter
    def name(self, value):
        if not (len(value.split()) >= 2 and all(word[0].isupper()
